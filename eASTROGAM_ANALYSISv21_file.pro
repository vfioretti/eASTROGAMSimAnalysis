; eASTROGAM_ANALYSISv21_file.pro - Description
; ---------------------------------------------------------------------------------
...
; ----------------------------------------------
; Usage:
; eASTROGAM_ANALYSISv21_file
; ---------------------------------------------------------------------------------
; Notes:
; Each THELSIM FITS files individually processed


pro eASTROGAM_ANALYSISv21_file
...
read, bogemms_tag, PROMPT='% - Enter BoGEMMS release (e.g. 211):'
...
read, pol_type, PROMPT='% - Is the source polarized? [0 = false, 1 = true]:'
...
if ((astrogam_version EQ 'V1.0') or (astrogam_version EQ 'V1.1')) then begin
    ...
    CREATE_STRUCT, rawData, 'rawData', ['EVT_ID', 'VOL_ID', 'MOTH_ID', 'TRAY_ID', 'PLANE_ID', 'STRIP_ID_X', 'STRIP_ID_Y', 'E_DEP', 'X_ENT', 'Y_ENT', 'Z_ENT', 'X_EXIT', 'Y_EXIT', 'Z_EXIT', 'CHILD_ID', 'PROC_ID'], $
    'I,I,J,I,I,I,I,F20.5,F20.5,F20.5,F20.5,F20.5,F20.5,F20.5,I,I', DIMEN = n_elements(event_id)
    ...
    if (isStrip) then begin
      ...
      while (1) do begin
         ...
         while(1) do begin
            ...
            if (where_pair(0) NE -1) then pair_flag_tot = [pair_flag_tot, 1] else pair_flag_tot = [pair_flag_tot, 0]
...
if ((astrogam_version EQ 'V1.0') or (astrogam_version EQ 'V1.1')) then begin
      ...
      openw,lun,outdir+'/'+sim_tag+'_CLUSTER_'+strmid(strtrim(string(N_in),1),0,10)+part_type+'_'+sname+'_'+ene_dis+'_'+ang_type+'_'+ene_type+'MeV_'+strmid(strtrim(string(theta_type),1),0,10)+'_'+strmid(strtrim(string(phi_type),1),0,10)+'.'+pol_string+strtrim(string(ifile),1)+'.dat',/get_lun
          ...
          pair_flag_x_array = MAKE_ARRAY(default_max_cols, N_trig, /INTEGER, VALUE = 0)
          pair_flag_y_array = MAKE_ARRAY(default_max_cols, N_trig, /INTEGER, VALUE = 0)
          ...
          while (1) do begin
              ...
              temp_pairx = dblarr(n_elements(where_clusterx))
              ...
              temp_pairy = dblarr(n_elements(where_clustery))
              ...
              pair_flag_temp = Glob_pair_flag_cluster(where_event_eq) 
              ...
              while (1) do begin
                  ...
                  pair_flag_temp_tray = pair_flag_temp(where_tray_eq)     
                  ...
                  while (1) do begin
                       ...
                       pair_flag_temp_plane = pair_flag_temp_tray(where_plane_eq)
                       ...
                       while (1) do begin
                           ...
                           if ((Si_id_temp_plane(last) EQ 2) or (Si_id_temp_plane(last) EQ 0)) then begin
                              for jel = 0l, n_elements(temp_samesi)-1 do begin
                                    ...
                                    temp_pairx[jel + counterx] = pair_flag_temp_tray(t)
                              endfor
                           ...
                           if ((Si_id_temp_plane(last) EQ 3) or (Si_id_temp_plane(last) EQ 1)) then begin
                              for jel = 0l, n_elements(temp_samesi)-1 do begin
                                    ...
                                    temp_pairy[jel + countery] = pair_flag_temp_tray(t)
                              endfor
                           ...
                           Glob_pair_flag_sum = [Glob_pair_flag_sum, pair_flag_temp_plane(last)]
             ...
             for jcol = 0, n_elements(temp_planex) -1 do begin 
                      ...
                      pair_flag_x_array[jcol, check_N_trig] = temp_pairx[jcol]
             endfor
             for jcol = 0, n_elements(temp_planey) -1 do begin 
                      ...
                      pair_flag_y_array[jcol, check_N_trig] = temp_pairy[jcol]
              endfor
      ...
      ;    cluster_y_array = cluster_y_array[keepcols, *]
          ...
          CREATE_STRUCT, KALMANTRACKER, 'TRACKERKALMAN', ['Event_ID', 'Theta', 'Phi','Energia','Piani_X','Clusters_X', 'Pair_X', 'Piani_Y','Clusters_Y', 'Pair_Y'], 'J,F,F,F,I('+string_dim+'),D('+string_dim+'),I('+string_dim+'),I('+string_dim+'),D('+string_dim+'),I('+string_dim+')', DIMEN = N_ELEMENTS(theta_kalman)
          ...
          KALMANTRACKER.Pair_X = pair_flag_x_array
          KALMANTRACKER.Piani_Y = plane_y_array
          KALMANTRACKER.Clusters_Y = cluster_y_array
          KALMANTRACKER.Pair_Y = pair_flag_y_array
          ...
          if (N_elements(trigger_index) GT 1) then begin
             ...
             pair_flag_x_array_trigger = pair_flag_x_array(*, trigger_index)
             pair_flag_y_array_trigger = pair_flag_y_array(*, trigger_index)
             
          endif
          
          CREATE_STRUCT, KALMANTRIGGER, 'TRIGGERKALMAN', ['Event_ID', 'Theta', 'Phi','Energia','Piani_X','Clusters_X','Pair_X', 'Piani_Y','Clusters_Y','Pair_Y'], 'J,F,F,F,I('+string_dim+'),D('+string_dim+'),I('+string_dim+'),I('+string_dim+'),D('+string_dim+'),I('+string_dim+')', DIMEN = N_ELEMENTS(theta_kalman_trigger)
          ...
          KALMANTRIGGER.Pair_X = pair_flag_x_array_trigger
          KALMANTRIGGER.Piani_Y = plane_y_array_trigger
          KALMANTRIGGER.Clusters_Y = cluster_y_array_trigger
          KALMANTRIGGER.Pair_Y = pair_flag_y_array_trigger
