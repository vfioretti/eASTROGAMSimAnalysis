...
for ifile=0, n_fits-1 do begin
  ...
  print, '%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%'
  print, '                             Tracker   '
  print, '           Saving the Tracker raw hits (fits)      '
  print, '%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%'
      ...
      while (1) do begin
        ...
        for r=0l, n_elements(Cluster_x_temp)-1 do begin
          if (e_dep_x_temp(r) GT E_th) then begin
            printf, lun, event_id(j), theta_type, phi_type, ene_type, plane_id_temp(r), Cluster_z_temp(r), 0, Cluster_x_temp(r), e_dep_x_temp(r), 1, child_temp(r), proc_temp(r), format='(I7,I7,I7,I7,I7,F20.7,I7,F20.7,F20.7,I7,I7,I7)'
          endif
        endfor
        ...
        for r=0l, n_elements(Cluster_y_temp)-1 do begin
          if (e_dep_y_temp(r) GT E_th) then begin
            printf, lun, event_id(j), theta_type, phi_type, ene_type, plane_id_temp(r), Cluster_z_temp(r), 1, Cluster_y_temp(r), e_dep_y_temp(r), 1, child_temp(r), proc_temp(r), format='(I7,I7,I7,I7,I7,F20.7,I7,F20.7,F20.7,I7,I7,I7)'
          endif
        endfor
    ...
    if (isStrip) then begin
      ...
      while (1) do begin
        ...
        while(1) do begin
          ...
          ; Searching for Pair/Compton events of secondary particles
  ...
  if (cal_flag EQ 1) then begin
    ...
    if (n_elements(event_id_cal) GT 0) then begin
      ...
      where_compton_cal = where((pair_flag_tot_cal EQ 2) or (pair_flag_tot_cal EQ 4))
      if (where_compton_cal(0) NE -1) then begin
        event_id_tot_cal_compton = event_id_tot_cal[where_compton_cal]
        bar_id_tot_compton = bar_id_tot[where_compton_cal]
        bar_ene_tot_compton = bar_ene_tot[where_compton_cal]
        pair_flag_tot_cal_compton = pair_flag_tot_cal[where_compton_cal]
      ...
      where_pair_cal = where((pair_flag_tot_cal EQ 1) or (pair_flag_tot_cal EQ 3))
      if (where_pair_cal(0) NE -1) then begin
        event_id_tot_cal_pair = event_id_tot_cal[where_pair_cal]
        bar_id_tot_pair = bar_id_tot[where_pair_cal]
        bar_ene_tot_pair = bar_ene_tot[where_pair_cal]
        pair_flag_tot_cal_pair = pair_flag_tot_cal[where_pair_cal]
  ...
  if (ac_flag EQ 1) then begin
    ...
    if (n_elements(event_id_ac) GT 0) then begin
      ...
      if (where_compton_ac(0) NE -1) then begin
        ...
        AC_subpanel_compton = AC_subpanel[where_compton_ac]
        energy_dep_tot_ac_compton = energy_dep_tot_ac[where_compton_ac]
        pair_flag_tot_ac_compton = pair_flag_tot_ac[where_compton_ac]
      ...
      if (where_pair_ac(0) NE -1) then begin
        ...
        AC_subpanel_pair = AC_subpanel[where_pair_ac]
        energy_dep_tot_ac_pair = energy_dep_tot_ac[where_pair_ac]
        pair_flag_tot_ac_pair = pair_flag_tot_ac[where_pair_ac]
